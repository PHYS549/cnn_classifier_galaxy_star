...
def display_patch(gal_patch, star_patch, title=""):
    """Helper function to display galaxy and star patches together."""
    ...
    for i, ax in enumerate(axes[0, 1:]):  # Start from the second column
        ax.imshow(gal_patch[:, :, i], cmap="gray")
        ax.set_title(f"{['i', 'r', 'g', 'z', 'u'][i]}-band")
        ax.axis('off')
    ...
    for i, ax in enumerate(axes[1, 1:]):  # Start from the second column
        ax.imshow(star_patch[:, :, i], cmap="gray")
        ax.set_title(f"{['i', 'r', 'g', 'z', 'u'][i]}-band")
        ax.axis('off')
